...
resource "google_compute_region_backend_service" "iap_internal" {
...
resource "google_compute_backend_service" "iap_external" {
  count                           = (var.enable_external_load_balancer && var.iap != null) ? 1 : 0
  name                            = "${var.name}-external-iap"
  protocol                        = "HTTP"
  port_name                       = local.atlantis_port_name
  timeout_sec                     = 10
  connection_draining_timeout_sec = 5
...
resource "google_compute_region_url_map" "default_internal" {
  name    = "${var.name}-internal"
  project = var.project
  region  = var.region

  # If IAP is not used, use default backend service for unmatched requests
  default_service = var.iap == null ? google_compute_region_backend_service.default.id : null
  ...
  dynamic "path_matcher" {
    ...
    content {
      name            = "public"
      default_service = google_compute_region_backend_service.iap_internal[0].id
      path_rule {
        paths   = ["/events"]
...
resource "google_compute_url_map" "default_external" {
  count           = (var.enable_external_load_balancer && var.iap != null) ? 1 : 0
  name            = "${var.name}-external"
  project         = var.project
  default_service = google_compute_backend_service.iap_external[0].id
}

resource "google_compute_region_target_https_proxy" "default_internal" {
  name    = "${var.name}-internal"
  region  = var.region
  url_map = google_compute_region_url_map.default_internal.id
  ssl_certificates = [
...
resource "google_compute_target_https_proxy" "default_external" {
  count   = (var.enable_external_load_balancer && var.iap != null) ? 1 : 0
  name    = "${var.name}-external"
  url_map = google_compute_url_map.default_external[0].id
  ssl_certificates = [
    google_compute_managed_ssl_certificate.default.id,
  ]
  ssl_policy = var.ssl_policy
...
resource "google_compute_forwarding_rule" "https_internal" {
  name                  = "${var.name}-internal"
  region                = var.region
  target                = google_compute_region_target_https_proxy.default_internal.id
  port_range            = "443"
  load_balancing_scheme = "INTERNAL_MANAGED"
  network               = var.network
  subnetwork            = var.subnetwork
...
resource "google_compute_global_forwarding_rule" "https_external" {
  count                 = (var.enable_external_load_balancer && var.iap != null) ? 1 : 0
  name                  = "${var.name}-external"
  target                = google_compute_target_https_proxy.default_external[0].id
  port_range            = "443"
